# Technical Spec: Модель данных заказа (Supabase Schemas & RLS)

> **Статус**: Проверено и зафиксировано (Группа 4: Данные, Auth и безопасность Supabase)
> **Дата создания**: 2026-07-21
> **Дата обновления**: 2026-09-01
> **Версия**: 1.6
> **Источник**: `../01_global_spec/Global_Spec.md`, `../02_functional_map/Functional_Map.md`, `../03_feature_specs/Feature_Order_Entry.md`, `../03_feature_specs/Feature_Order_Statuses.md`, `../03_feature_specs/Feature_Menu_Management.md`, `../03_feature_specs/Feature_Kitchen_Ticket.md`, `../03_feature_specs/Feature_Payment_Flow.md`, `Technical_MVP_Implementation_Decisions.md`

> **[Правка Блока 2 — 2026-08-29]** Правка под пункт 7 плана
> `../../work_plans/active/Work_Plan_Block_2_Ops_Auth_Polish.md`: поле
> `chat_access_code_hash` в `ClientOrderAccess` для кросс-девайс доступа к заказу и чату
> по паре «номер заказа + код» (§13, §15 п.10). Аддитивная миграция.

> **[Правка — 2026-09-01]** Эпик D плана
> `../../work_plans/completed/Work_Plan_User_Stories_Coverage_Alignment.md`: идемпотентность
> финальной отправки (§3) и отказ браузера в геолокации как штатный путь со ссылкой
> Google Maps (§7). Изменения уточняющие, набор сущностей и полей не меняется.

> **[Правка — 2026-09-01]** Эпик E того же плана: срок и семантика доступа по паре
> «номер + код» (`expires_at` = закрытие + 30 дней), перевыпуск кода менеджером,
> доступ по одному заказу за раз (§13, §15 пп. 13–14). Новые поля не добавляются.

---

## 1. Назначение

Документ описывает техническую модель данных заказа: схемы PostgreSQL в **Supabase**, RLS-политики доступа, таблицы, индексы, порядок миграций (`supabase/migrations/*.sql`) и механизмы append-only триггеров в рамках Группы 4 плана `../../work_plans/active/Work_Plan_Code_Readiness.md`.

---

## 1.1 Схемы данных Supabase и порядок миграций

Служебные схемы Supabase:

1. **`public` schema**: Доменные таблицы, к которым обращаются клиенты через Supabase JS Client под защитой Supabase RLS policies (`orders`, `order_items`, `menu_items`, `client_order_access`, `complaints`, `chat_messages`).
2. **`private` schema**: Внутренние служебные таблицы, недоступные для прямого клиентского обращения (`verification_requests`, `trusted_channels`, `audit_logs`, `employee_profiles`, `resolution_records`). Обращение выполняется только через Server Actions с использованием Service Role Key.

Порядок накатки миграций (`supabase/migrations/*.sql`):

1. **`00001_enum_types.sql`**: Создание пользовательских типов `ENUM` (`user_role`, `order_status`, `payment_status`, `employee_status`, `complaint_category`, `complaint_status`, `risk_level`, `delivery_assignment_status`, `kitchen_ticket_status`).
2. **`00002_core_tables.sql`**: Создание таблиц `public` и `private` схем с внешними ключами и ограничениями `CHECK`.
3. **`00003_enable_rls.sql`**: Включение Row Level Security (`ALTER TABLE ... ENABLE ROW LEVEL SECURITY;`) по принципу **`default deny`**.
4. **`00004_rls_policies.sql`**: Создание правил RLS для ролей Supabase Auth (`authenticated`, `anon`).
5. **`00005_indexes_and_triggers.sql`**: Создание индексов, уникальных ограничений и append-only триггеров.

---

## 2. Основные сущности

Минимальный набор сущностей:

1. `Order` - рабочий заказ.
2. `OrderDraft` - черновик или неподтвержденная попытка.
3. `OrderItem` - позиция заказа.
4. `MenuItemSnapshot` - снимок блюда на момент добавления в корзину.
5. `CustomerContact` - контактные данные клиента.
6. `DeliveryInfo` - адрес, ориентир, геолокация.
7. `OrderStatusHistory` - история статусов.
8. `OrderEventLog` - журнал событий заказа.
9. `PaymentRecord` - способ и состояние оплаты.
10. `DeliveryAssignment` - назначение курьера.
11. `KitchenTicket` - кухонный чек.
12. `ClientChatThread` - чат клиента по заказу.
13. `ClientOrderAccess` - токен доступа устройства клиента.

---

## 3. Order

`Order` создается только после финальной отправки подтвержденного заказа.

Одна финальная отправка подтвержденного `OrderDraft` создает ровно один `Order`. Повторная или конкурентная финальная отправка того же черновика (двойное нажатие, повтор запроса на медленной сети, перезагрузка страницы) возвращает уже созданный `Order` с тем же `orderNumber` и не порождает второй заказ, второе автоматическое сообщение в чат заказа или вторую запись `OrderStatusHistory`. Черновик после создания заказа сохраняется как связанный технический источник по §15 п.2.

Минимальные поля:

* `id`;
* `orderNumber`;
* `createdAt`;
* `updatedAt`;
* `currentStatus`;
* `customerContactId`;
* `deliveryInfoId`;
* `paymentRecordId`;
* `source`;
* `managerId`;
* `courierId`;
* `totalAmount`;
* `currency`;
* `isDiscountOrder`;
* `sourceReturnedOrderId` (nullable);
* `riskWarningLevel`;
* `cookingStartedAt` (`cooking_started_at`, nullable до перехода в `Готовится`);
* `cookingCompletedAt` (`cooking_completed_at`, nullable до готовности);
* `cookingTargetMinutes` (`cooking_target_minutes`, норматив, зафиксированный при старте приготовления);
* `closedAt`.

`orderNumber` должен быть понятным клиенту и использоваться для клиентского чата и отслеживания.

---

## 4. OrderDraft

`OrderDraft` нужен для корзины, незавершенного оформления и неподтвержденных попыток.

Минимальные состояния:

* `Черновик`;
* `Ожидает подтверждения`;
* `Канал подтвержден`;
* `Неподтвержденная попытка`;
* `Архив`.

Если клиент не подтвердил Telegram или Email, `OrderDraft` не превращается в `Order`.

Состояние обработки такой попытки хранится в отдельном связанном `ResolutionCase`, а не в `OrderDraft.status`.

---

## 5. OrderItem и снимок цены

Каждая позиция заказа должна хранить снимок данных блюда на момент добавления в корзину.

Минимальные поля `OrderItem`:

* `id`;
* `orderId` или `draftId`;
* `menuItemId`;
* `nameSnapshot`;
* `priceSnapshot`;
* `finalPrice`;
* `quantity`;
* `modifiers`;
* `customerComment`;
* `availabilityStatusAtSubmit`.

Правило цены:

* если цена выросла после добавления в корзину, для клиента сохраняется старая цена;
* если цена снизилась или появилась скидка, клиент получает более выгодную цену;
* если блюдо стало недоступным, заказ нельзя отправить, пока клиент не удалит или не заменит позицию.

---

## 6. CustomerContact

Сущность хранит контактные данные, но доступ к ним регулируется `Technical_Access_Audit.md`.

Минимальные поля:

* `name`;
* `phone`;
* `telegram`;
* `email`;
* `optionalMessage`;
* `optionalWhatsApp`;
* `genderOrContactFormat`;
* `trustedChannelId`;
* `createdAt`;
* `updatedAt`.

Пол или формат обращения не должны показываться менеджеру и курьеру как обычное поле заказа.

---

## 7. DeliveryInfo

Минимальные поля:

* `address`;
* `landmark`;
* `geoLocation`;
* `googleMapsLink`;
* `deliveryNotes`;
* `locationPrecision`;
* `createdAt`.

Геолокация обязательна для доставки. Техническая реализация должна позволять хранить как координаты, так и ссылку Google Maps, если клиент передал ее вручную.

Отказ браузера предоставить геолокацию — штатная ситуация оформления, а не ошибка. Если координаты недоступны, клиент указывает точку доставки ссылкой Google Maps; заполненная `googleMapsLink` удовлетворяет требование обязательной геолокации, а `locationPrecision` фиксирует, что источник — ручная ссылка. Если координаты все же получены, они остаются основным источником по §15 п.4.

---

## 8. OrderStatusHistory

История статусов должна быть append-only: новые события добавляются, старые не перезаписываются.

Минимальные поля:

* `orderId`;
* `fromStatus`;
* `toStatus`;
* `changedBy`;
* `changedByRole`;
* `changedAt`;
* `reason`;
* `source`.

Текущий статус заказа может храниться отдельно для быстрого доступа, но история должна сохраняться.

---

## 9. PaymentRecord

`PaymentRecord` хранит способ и состояние оплаты независимо от `Order.currentStatus`.

Минимальные поля:

* `orderId`;
* `paymentMethod`;
* `paymentStatus`;
* `amount`;
* `confirmedBy`;
* `confirmedAt`.

Минимальные состояния `paymentStatus`: `Не подтверждена` и `Подтверждена`.

Изменение `paymentStatus` не переводит заказ в `Доставлен` и не меняет клиентский этап. Переход заказа в `Закрыт` проверяет одновременно подтвержденную доставку и `paymentStatus = Подтверждена`.

---

## 10. OrderEventLog

`OrderEventLog` фиксирует события, которые не всегда являются статусами:

* печать кухонного чека;
* повторная печать чека;
* раскрытие клиентских данных;
* отправка платежной ссылки;
* подтверждение оплаты;
* сообщение в чат;
* передача в урегулирование;
* риск-предупреждение;
* назначение курьера;
* QR/fallback подтверждение;
* перевыпуск кода доступа клиента менеджером.

---

## 11. DeliveryAssignment

**Назначение курьера**

| Поле | Тип | Обязательное | Описание |
|------|-----|-------------|----------|
| id | uuid | да | PK, gen_random_uuid() |
| order_id | uuid | да | FK → orders.id |
| courier_id | uuid | да | FK → auth.users.id (роль COURIER) |
| status | enum delivery_assignment_status | да | ASSIGNED, IN_TRANSIT, ARRIVED, COMPLETED, CANCELLED |
| assigned_at | timestamptz | да | Момент назначения |
| picked_up_at | timestamptz | нет | Момент забора заказа курьером |
| arrived_at | timestamptz | нет | Момент прибытия к клиенту |
| completed_at | timestamptz | нет | Момент завершения доставки |
| cancelled_at | timestamptz | нет | Момент отмены назначения |
| cancel_reason | text | нет | Причина отмены/переназначения |
| confirmation_token_hash | text | нет | SHA-256 хэш QR-токена для подтверждения доставки |
| confirmation_token_used | boolean | да | default false, одноразовое гашение |
| route_group_id | uuid | нет | FK для попутной доставки (если объединены заказы) |

---

## 12. KitchenTicket

**Кухонный чек**

| Поле | Тип | Обязательное | Описание |
|------|-----|-------------|----------|
| id | uuid | да | PK, gen_random_uuid() |
| order_id | uuid | да | FK → orders.id |
| ticket_number | serial | да | Человеко-читаемый номер чека |
| printed_at | timestamptz | да | Момент печати / передачи на кухню |
| printed_by | uuid | да | FK → auth.users.id (менеджер, который передал) |
| status | enum kitchen_ticket_status | да | PRINTED, COOKING, READY |
| marked_ready_at | timestamptz | нет | Момент фиксации готовности менеджером или поваром |
| marked_ready_by | uuid | нет | FK → auth.users.id (менеджер или повар, который отметил) |

`KitchenTicket` фиксирует бумажный артефакт, если менеджер сформировал или распечатал чек. KDS строит очередь из рабочего `Order`, его позиций и снимков меню, поэтому электронный путь не требует фиктивной печати. Бумажный и электронный пути сходятся в одном lifecycle: переход в `Готовится` фиксирует `cooking_started_at` и норматив, а переход в `Готов к доставке` однократно фиксирует `cooking_completed_at`.

Повторная команда готовности для уже готового заказа возвращает существующий результат и не добавляет вторую запись в `OrderStatusHistory` или `OrderEventLog`. `actualMinutes` рассчитывается из сохраненных серверных меток времени и не хранится как независимо изменяемое поле.

---

## 13. ClientOrderAccess

**Токен доступа устройства клиента**

| Поле | Тип | Обязательное | Описание |
|------|-----|-------------|----------|
| id | uuid | да | PK, gen_random_uuid() |
| order_id | uuid | да | FK → orders.id |
| device_token_hash | text | да | SHA-256 хэш токена, хранящегося на устройстве клиента |
| chat_access_code_hash | text | да | **(Блок 2)** Хэш короткого кода доступа (напр. 4 цифры), выданного клиенту после подтверждения канала. Вместе с `orderNumber` открывает заказ и чат с любого устройства |
| created_at | timestamptz | да | Момент создания |
| expires_at | timestamptz | да | Момент истечения доступа |
| last_accessed_at | timestamptz | нет | Последний визит клиента на страницу заказа |

Примечание: Токен хранится на устройстве клиента (localStorage). В БД хранится только хэш. Персональных данных клиента в этой таблице нет.

**(Блок 2)** `chat_access_code` в открытом виде показывается клиенту один раз при
оформлении заказа (заметно, с подписью «сохраните код»); в БД — только хэш. Проверка
пары «`orderNumber` + код» выполняется серверной RPC-функцией под `default deny` RLS,
без открытого `SELECT` по таблице заказов. Код генерируется только после успешного
подтверждения Telegram/Email (OTP).

**(Эпик E)** Одна запись `ClientOrderAccess` соответствует одному заказу; проверка пары
«`orderNumber` + код» открывает ровно один заказ и не возвращает список других заказов
клиента. `expires_at` задаётся так: пока заказ не закрыт — доступ активен; при закрытии
заказа `expires_at` устанавливается на 30 дней от закрытия, что совпадает со сроком
жизни чата. После `expires_at` RPC доступа отклоняет пару «номер + код», а история
остаётся для внутреннего аудита.

**(Эпик E)** Перевыпуск кода: менеджер из карточки заказа в CRM генерирует новый
`chat_access_code`, серверная операция перезаписывает `chat_access_code_hash` (старый
хэш не сохраняется), новый код показывается менеджеру один раз для передачи клиенту, а
событие пишется в `OrderEventLog`. Прежний код после перевыпуска не проходит проверку.

---

## 14. Связи с другими сущностями

Заказ должен быть связан с:

* токеном доступа устройства клиента (ClientOrderAccess);
* назначением курьера (DeliveryAssignment);
* кухонным чеком;
* платежной записью;
* доставкой;
* клиентским чатом;
* жалобой, если она создана по этому заказу;
* ситуацией урегулирования, если она создана;
* маршрутной группой, если заказ идет в попутной доставке;
* скидочной продажей, если заказ возвращен и выставлен со скидкой.

При повторной продаже возвращенного заказа исходный `Order` не переиспользуется для нового клиента. После подтвержденной покупки создается новый `Order`:

* `isDiscountOrder = true`;
* `sourceReturnedOrderId` указывает на исходный возвращенный заказ;
* новый заказ получает собственные `CustomerContact`, `DeliveryInfo`, `PaymentRecord`, `DeliveryAssignment`, чат и историю;
* начальный операционный статус нового заказа — `Готов к доставке`, потому что товар уже приготовлен;
* исходный заказ переходит в `Продан со скидкой` и сохраняет собственную историю и данные исходного клиента.

Связь должна предотвращать смешивание контактов, оплаты и доставки двух клиентов.

---

## 15. Закрытые решения

1. `orderNumber` должен иметь формат с датой и коротким порядковым номером за день, например `20260721-0042`. Такой формат понятен клиенту и снижает риск раскрытия общего количества заказов за все время.
2. `OrderDraft` хранится после создания `Order` как связанный технический источник, но не используется как рабочий заказ. Это нужно для аудита подтверждения, корзины и спорных ситуаций.
3. `CustomerProfile` отделяется от `CustomerContact` уже на первом этапе. Профиль нужен для доверенных каналов, истории риска и повторного заказа, а контактные данные должны управляться отдельно и безопасно.
4. Для доставки обязательны координаты геолокации. Ссылка Google Maps может храниться дополнительно как удобный источник, но не должна заменять координаты, если координаты технически получены.
5. Полную историю каждого мелкого изменения корзины хранить не нужно. Обязательно хранится итоговый состав заказа, снимки цен и ключевые события: добавление в корзину, финальная отправка, недоступная позиция, согласованная корректировка менеджером.
6. Состояние оплаты хранится в `PaymentRecord` отдельно от статуса заказа. `Закрыт` разрешен только при одновременном наличии подтвержденной доставки и оплаты.
7. `В урегулировании` является состоянием отдельного `ResolutionCase`, а не `OrderDraft`.
8. Повторная продажа создает отдельный `Order` со ссылкой `sourceReturnedOrderId`; данные исходного и нового клиента не объединяются.
9. **Решение: В MVP отдельная таблица `CustomerProfile` не создаётся.** Все контактные данные клиента привязаны к конкретному заказу через `CustomerContact`. Нормализация и сопоставление клиентов между заказами выполняется через `normalized_phone_e164` и `normalized_telegram_username` в `CustomerContact`. Отдельный профиль клиента — это backlog для будущих версий.
10. **(Блок 2)** Код доступа клиента к заказу (`chat_access_code`) — открытый 4-значный код, генерируется только после подтверждения канала OTP, показывается клиенту один раз при оформлении; в БД хранится только SHA-256 хэш. Пара «`orderNumber` + код» открывает заказ через RPC-функцию `get_order_by_access` (`SECURITY DEFINER`), а не открытым `SELECT` — форма работает и при выключенной RLS, и после её восстановления в группе D. **В текущей упрощённой боевой схеме** (`supabase/full_schema.sql`: таблицы `dishes`, `orders`, `order_chat_messages`; таблицы `ClientOrderAccess` в проде ещё нет) хэш лежит колонкой `orders.chat_access_code_hash` (миграция `00011_orders_chat_access_code.sql`, аддитивная). При вводе полной модели `ClientOrderAccess` (группа D / RLS) поле переносится туда.
11. **(Эпик D)** Одна финальная отправка подтвержденного `OrderDraft` создает ровно один `Order`; повторная и конкурентная отправка того же черновика идемпотентно возвращают уже созданный заказ без второго `Order`, автосообщения в чат и записи истории (§3).
12. **(Эпик D)** Отказ браузера предоставить геолокацию — штатный путь оформления: ручная ссылка Google Maps (`googleMapsLink`) закрывает требование обязательной геолокации, `locationPrecision` фиксирует источник; при наличии координат приоритет за ними по п.4 (§7).
13. **(Эпик E)** Пара «`orderNumber` + код» открывает ровно один заказ; кросс-девайс список заказов клиента не строится. `expires_at` записи `ClientOrderAccess` = момент закрытия заказа + 30 дней (совпадает со сроком чата); до закрытия доступ активен. После `expires_at` RPC доступа отклоняет пару (§13).
14. **(Эпик E)** Менеджер перевыпускает `chat_access_code` из карточки заказа: новый хэш перезаписывает старый, новый код показывается менеджеру один раз, событие пишется в `OrderEventLog`; прежний код перестаёт действовать (§13).
