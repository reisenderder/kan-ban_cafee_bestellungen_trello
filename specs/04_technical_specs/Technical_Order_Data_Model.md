...
10. **(Блок 2)** `ClientOrderAccess` получает поле `chat_access_code_hash`. Открытый код (напр. 4 цифры) генерируется только после подтверждения канала OTP, показывается клиенту один раз при оформлении, в БД хранится хэш. Пара «`orderNumber` + код» открывает заказ и чат с любого устройства через RPC-функцию под `default deny` RLS. Аддитивная миграция (`ALTER TABLE ... ADD COLUMN`), обратно совместимая.
